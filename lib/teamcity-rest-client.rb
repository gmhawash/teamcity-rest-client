...
module TeamcityRestClient
  ...
  Change = Struct.new(:teamcity, :id, :version, :href) do
...
class Teamcity
  ...
  def changes options = {}
    doc(get('/app/rest/changes', options)).elements.collect('//change') do |e|
      TeamcityRestClient::Change.new(self, e.av("id"), e.av("version"), url(e.av("href")))
    end
  end
